...
/**
 * Created By Rapharino on 2017/10/31 下午8:37
 *
 * Metric Registry
 */
public class MetricsRegister extends MetricRegistry {

    public static final MetricsRegister Metrics = new MetricsRegister();
    ...
    public Counter counter(MetricsRegister.MetricName name) {
    ...
    public Counter counter(MetricsRegister.MetricName name, MetricsRegister.MetricName alias) {
        Counter counter = counter(name);
        registerAlias(name, alias);
        return counter;
    }

    public Meter meter(MetricsRegister.MetricName name) {
    ...
    public Meter meter(MetricsRegister.MetricName name, MetricsRegister.MetricName alias) {
    ...
    public <T extends Metric> T register(MetricsRegister.MetricName name, T metric) {
        try {
    ...
    public <T extends Metric> T register(MetricsRegister.MetricName name, MetricsRegister.MetricName aliasName,
                                         T metric) {
    ...
    public boolean remove(MetricsRegister.MetricName name) {
    ...
    public boolean remove(MetricsRegister.MetricName name, MetricsRegister.MetricName alias) {
        if (remove(name)) {
            removeAlias(alias);
            return true;
        }
    ...
    public void registerMBean(Metric metric, ObjectName name) {
        MetricsRegister.AbstractBean mbean;

        if (metric instanceof Gauge) {
            mbean = new MetricsRegister.JmxGauge((Gauge<?>) metric, name);
        } else if (metric instanceof Counter) {
            mbean = new MetricsRegister.JmxCounter((Counter) metric, name);
        } else if (metric instanceof Histogram) {
            mbean = new MetricsRegister.JmxHistogram((Histogram) metric, name);
        } else if (metric instanceof Meter) {
            mbean = new MetricsRegister.JmxMeter((Meter) metric, name, TimeUnit.SECONDS);
        } else if (metric instanceof Timer) {
            mbean = new MetricsRegister.JmxTimer((Timer) metric, name, TimeUnit.SECONDS, TimeUnit.MICROSECONDS);
        } else {
    ...
    private void registerAlias(MetricsRegister.MetricName existingName, MetricsRegister.MetricName aliasName) {
    ...
    private void removeAlias(MetricsRegister.MetricName name) {
        try {
            mBeanServer.unregisterMBean(name.getMBeanName());
        } catch (Exception ignore) {
        }
    ...
    private abstract static class AbstractBean implements MetricsRegister.MetricMBean {
    ...
    public interface JmxGaugeMBean extends MetricsRegister.MetricMBean {

        Object getValue();
    }

    private static class JmxGauge extends MetricsRegister.AbstractBean implements MetricsRegister.JmxGaugeMBean {
    ...
    public interface JmxHistogramMBean extends MetricsRegister.MetricMBean {
    ...
    private static class JmxHistogram extends MetricsRegister.AbstractBean
            implements MetricsRegister.JmxHistogramMBean {
    ...
    public interface JmxCounterMBean extends MetricsRegister.MetricMBean {

        long getCount();
    }

    private static class JmxCounter extends MetricsRegister.AbstractBean implements MetricsRegister.JmxCounterMBean {
    ...
    public interface JmxMeterMBean extends MetricsRegister.MetricMBean {
    ...
    private static class JmxMeter extends MetricsRegister.AbstractBean implements MetricsRegister.JmxMeterMBean {
    ...
    public interface JmxTimerMBean extends MetricsRegister.JmxMeterMBean {
    ...
    static class JmxTimer extends MetricsRegister.JmxMeter implements MetricsRegister.JmxTimerMBean {
     ...
     */
    public static class MetricName implements Comparable<MetricsRegister.MetricName> {
        ...
        /**
         * Creates a new {@link MetricsRegister.MetricName} without a scope.
         *
        ...
        public MetricName(Class<?> klass, String name, String scope) {
            this(klass.getPackage() == null ? "" : klass.getPackage().getName(),
                    withoutFinalDollar(klass.getSimpleName()), name, scope);
        }
         ...
         * @param name the name of the {@link Metric}
         * @param scope the scope of the {@link Metric}
         * @param mBeanName the 'ObjectName', represented as a string, to use when
         *            registering the
         *            MBean.
         */
         ...
         * metrics, this will be
         * the simple class name of the {@link Class} to which the {@link Metric}
         * belongs.
         *
        ...
        public boolean equals(Object o) {
            ...
            final MetricsRegister.MetricName that = (MetricsRegister.MetricName) o;
        ...
        @Override
        public int compareTo(MetricsRegister.MetricName o) {
